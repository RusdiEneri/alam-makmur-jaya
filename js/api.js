...
async function createReceivable(data) {
  ...
  const payload = {
    ...
    namaPelanggan: data.namaPelanggan || data.nama,
    noWhatsapp: data.noWhatsapp || data.wa,
    total: data.total,
    batasKredit: data.batasKredit,
    jatuhTempo: data.jatuhTempo,
    catatan: data.catatan
  };
  return apiFetch('/receivables', { method: 'POST', body: JSON.stringify(payload) });
}

async function bayarPiutang(id, jumlahBayar) {
  return apiFetch(`/receivables/${id}/bayar`, { method: 'PUT', body: JSON.stringify({ jumlahBayar }) });
}
...
async function createDelivery(data) {
  ...
  const payload = {
    ...
    namaPelanggan: data.namaPelanggan || data.namaPenerima,
    alamat: data.alamat,
    tanggalKirim: data.tanggalKirim || data.jadwalKirim,
    kurir: data.kurir,
    kendaraan: data.kendaraan,
    catatan: data.catatan
  };
  return apiFetch('/deliveries', { method: 'POST', body: JSON.stringify(payload) });
}

async function updateStatusPengiriman(id, status) {
  return apiFetch(`/deliveries/${id}/status`, { method: 'PUT', body: JSON.stringify({ status }) });
}
...
async function createReturn(data) {
  ...
  const payload = {
    ...
    productId: data.productId || data.produkId,
    namaProduk: data.namaProduk || 'Produk Retur', // Fallback
    qty: data.qty,
    alasan: data.alasan,
    kondisi: data.kondisi,
    keterangan: data.keterangan || data.tindakan
  };
