...
function write(name, data) {
  ...
  try {
    ...
    // Tulis ke file sementara untuk mencegah korupsi data
    fs.writeFileSync(tempPath, jsonStr, 'utf-8');
    
    // Validasi parse ulang
    ...
    console.error(`[ERROR] Gagal menyimpan data ${name}:`, e);
  ...
  try {
    ...
    for (const file of tempFiles) {
      if (fs.existsSync(file.tempPath)) {
        try { fs.unlinkSync(file.tempPath); } catch (err) {}
      }
    }
